...
Let's consider the weights of the last RNN and fully connected layers as $\theta \equiv (W_{xx}^L, \textcolor{red}{\cancel{W_{xu}^L}}, b_x^L, W_y, b_y)$.
...
Given a dataset of samples $(u_{0:T}^{(i)}, y_{0:T}^{(i)})_{i=1}^m$, we consider a training of our model as defined in \ref{model_definition} without the added weights, resulting in an initial set of weights $\theta_0$.
